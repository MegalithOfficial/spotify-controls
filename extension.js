...
var SpotifyIndicator = GObject.registerClass(
  class SpotifyIndicator extends PanelMenu.Button {
    ...
    _init(extensionPath, controlsPosition, settings) {
      ...
      this._showIconChangedId = this._settings.connect(
        "changed::show-spotify-icon",
        this._onShowIconChanged.bind(this),
      );
      this._showArtistChangedId = this._settings.connect(
        "changed::show-artist",
        this._onTrackDisplaySettingsChanged.bind(this),
      );
      this._showTitleChangedId = this._settings.connect(
        "changed::show-title",
        this._onTrackDisplaySettingsChanged.bind(this),
      );
    }
    ...
    _buildUI(extensionPath) {
      ...
      this.spotifyIcon.visible =
        this._settings.get_boolean("show-spotify-icon");
      ...
      this.trackLabel.visible =
        this._settings.get_boolean("show-artist") ||
      ...
      logDebug(
        "UI built with controls positioned to the " + this.controlsPosition,
      );
    }
      ...
      if (this.spotifyIcon) {
        ...
        this.spotifyIcon = new St.Icon({
          gicon: Gio.icon_new_for_string(
            `${this.extensionPath}/icons/spotify.svg`,
          ),
          icon_size: 16,
          style_class: "spotify-icon",
        });
    ...
    /**
     * Callback function when 'show-artist' or 'show-title' settings change.
     * Updates the label text and visibility.
     */
    _onTrackDisplaySettingsChanged() {
      logDebug("Track display settings changed");
      if (this._currentMetadata) {
        this._updateTrackInfo(this._currentMetadata);
      } else {
        // Even if no metadata, we might need to hide/show the label (e.g. "No Track Playing" or empty)
        // But for now, let's just rely on _updateTrackInfo handling null if we passed it, or just updating visibility.
        const showArtist = this._settings.get_boolean("show-artist");
        const showTitle = this._settings.get_boolean("show-title");
        if (this.trackLabel) {
          this.trackLabel.visible = showArtist || showTitle;
        }
      }
    }
    ...
    _activateSpotifyWindow() {
      ...
      const minimizeOnSecondClick = this._settings.get_boolean(
      ...
      for (let actor of windowActors) {
        ...
        logDebug(
          `Window WM_CLASS: ${JSON.stringify(wmClass)} (Type: ${typeof wmClass})`,
        );
        logDebug(
          `Window Title: ${window.get_title()} (Type: ${typeof window.get_title()})`,
        );
        logDebug(
          `Window Workspace: ${window.get_workspace()} (Type: ${typeof window.get_workspace()})`,
        );
      ...
      if (!spotifyFound) {
        logDebug(
          "Spotify window not found. Attempting to launch Spotify to show its window.",
        );
        ...
        try {
          ...
          logError(
            e,
            "Error while attempting to launch Spotify subprocess.",
            e,
          );
        }
      }
    ...
    _updateTrackInfo(metadata) {
      ...
      this.trackLabel.text = text;
      logDebug(`Updated track info: ${text}`);
    }
    ...
    destroy() {
      ...
      if (this._showArtistChangedId) {
        this._settings.disconnect(this._showArtistChangedId);
        this._showArtistChangedId = null;
      }

      if (this._showTitleChangedId) {
        this._settings.disconnect(this._showTitleChangedId);
        this._showTitleChangedId = null;
      }
...
export default class SpotifyControlsExtension extends Extension {
  ...
  enable() {
    ...
    this._controlsPositionChangedId = this._settings.connect(
      "changed::controls-position",
      this._onSettingsChanged.bind(this),
    );
    this._showControlsChangedId = this._settings.connect(
      "changed::show-playback-controls",
      this._onSettingsChanged.bind(this),
    );
    this._volumeControlChangedId = this._settings.connect(
      "changed::enable-volume-control",
      this._onSettingsChanged.bind(this),
    );
    this._showSpotifyIconChangedId = this._settings.connect(
      "changed::show-spotify-icon",
      this._onSettingsChanged.bind(this),
    );
  ...
  _updateIndicator() {
    ...
    switch (position) {
      ...
      case "middle-left":
        ...
        offset = Math.max(
          0,
          Math.floor(Main.panel._centerBox.get_children().length / 2) - 1,
        );
        break;
      case "center":
        boxName = "center";
    ...
    logDebug(
      `Indicator added at position: ${position}, box: ${boxName}, offset: ${offset}`,
    );
  }
